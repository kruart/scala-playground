...
sealed trait Set[+Element] /*extends (Element => Boolean)*/ {
  ...
  final /*override*/ def apply[Super >: Element](input: Super): Boolean = contains(input)
  final def doesNotContain[Super >: Element](input: Super): Boolean = !contains(input)
  final def contains[Super >: Element](input: Super): Boolean = exists(_ == input)
  final def doesNotExist(predicate: Element => Boolean): Boolean = !exists(predicate)
  final def exists(predicate: Element => Boolean): Boolean = fold(false)(_ || predicate(_))
  final def notForall(predicate: Element => Boolean): Boolean = !forall(predicate)
  final def forall(predicate: Element => Boolean): Boolean = fold(true)(_ && predicate(_))

  final def add[Super >: Element](input: Super): Set[Super] =
    fold(NonEmpty(input, empty)) { (acc, current) =>
      if (current == input) acc
      else NonEmpty(current, acc)
    }
  ...
  final def isSubsetOf(predicate: Element => Boolean): Boolean = forall(predicate)
  ...
  final override def toString: String =
    if (isEmpty)
      ...
      val otherElementsSplitByCommaSpace =
        otherElementsOrThrowException.fold(elementOrThrowException.toString) { (acc, current) =>
          s"$acc, $current"
        }
      s"{ $otherElementsSplitByCommaSpace }"
    }

  final def size: Int = fold(0) { (acc, _) => acc + 1 }
  final def isEmpty: Boolean = this.isInstanceOf[Empty.type]
  final def nonEmpty: Boolean = !isEmpty
  final def isSingleton: Boolean = nonEmpty && otherElementsOrThrowException.isEmpty
  ...
  final def flatMap[Result](function: Element => Set[Result]): Set[Result] =
    fold[Set[Result]](empty) { (acc, current) => function(current).fold(acc)(_ add _) }
