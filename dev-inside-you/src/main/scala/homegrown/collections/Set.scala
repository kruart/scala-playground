...
sealed trait Set[Element] extends (Element => Boolean) {
  ...
  final override def apply(input: Element): Boolean = contains(input)
  final def doesNotContain(input: Element): Boolean = !contains(input)
  final def contains(input: Element): Boolean = exists(_ == input)
  ...
  final def flatMap[Result](function: Element => Set[Result]): Set[Result] =
    fold(empty[Result]) { (acc, current) => function(current).fold(acc)(_ add _) }
