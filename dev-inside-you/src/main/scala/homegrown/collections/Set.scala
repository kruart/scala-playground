...
sealed abstract class Set[+Element] extends FoldableFactory[Element, Set] {
  ...
  final override protected def factory: Factory[Set] = Set
  final def apply[Super >: Element](input: Super): Boolean = contains(input)
  ...
  final def union[Super >: Element](that: Set[Super]): Set[Super] = fold(that)(_ add _)
...
object Set extends Factory[Set] {
  private final case class NonEmpty[+Element](left: Set[Element], element: Element, right: Set[Element]) extends Set[Element] {
    final def isSingleton: Boolean = left.isEmpty && right.isEmpty
    final override def sample: Option[Element] = Some(element)
  ...
  private object Empty extends Set[Nothing] {
    /** case Empty => causes stack overflows in methods like fold
      * case Empty() => does not
      */
    def unapply[Element](set: Set[Element]): Boolean = set.isInstanceOf[Empty.type]
    final override def isSingleton: Boolean = false
    final override def sample: Option[Nothing] = None
    final override def toString: String = "{}"
  }
